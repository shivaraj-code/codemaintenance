...
import com.io.codesystem.pharmacy.PharmacyDataVerificationModel;
...
public class MedicineMaintenanceService extends CodeMaintenanceService {
	...
	private List<MedicineStandardModel> prepareTargetCodeEntityFromInputStream(InputStream targetDataFileStream)
			throws IOException {
	...
	public void saveMedicineStandardList(List<MedicineStandardModel> medicineStandardList, String newTableName,
		...
		try {
			...
			asyncTasksStatusService.saveAsyncTaskStatusLog(fileId, "Dump Table Created",
					"Dump Table Created Successfully", "In Process", 60, userId);
...
//	        icdSynchResultsRepository.icdCompareAndSyncTables(fileId, taregtFileName, userId);
			try {
				...
				codeMaintenanceLoggerService.saveCodeMaintenanceLog(fileId, "Medicine Synching Completed",
						"Medicine Synching Completed Successfully", userId);
			} catch (Exception e) {
			...
			codeMaintenanceFile.setComments("Medicine File Proceessed Successfully");
			Timestamp newModifiedDate = Timestamp.valueOf(LocalDateTime.now());
			codeMaintenanceFile.setModifiedDate(newModifiedDate);
			codeMaintenanceFileService.saveCodeMaintenanceFile(codeMaintenanceFile);
			...
			return new CustomResponse("Medicine File Synched Successfully", "", HttpStatus.OK);
		} else {
			...
			asyncTasksStatusService.saveAsyncTaskStatusLog(fileId, "File Sync", "Code Sync process Failed", "Failed", 0,
					userId);
			return new CustomResponse("Medicine File Synch Failed", "", HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	protected CustomResponse saveCodeVerificationLogDetails(CodeVerfication codes, String codeset, int fileId,int userId,String notes) {
		...
		return new CustomResponse("Medicine Verification Log Saving Success", "", HttpStatus.OK);
	}
