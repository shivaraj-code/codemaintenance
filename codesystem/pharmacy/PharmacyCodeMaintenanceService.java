...
public class PharmacyCodeMaintenanceService extends CodeMaintenanceService {
	...
	protected void processData(int fileId, int userId) {
		processVerifiedData(fileId, userId);
	}

	protected CustomResponse processVerifiedData(int fileId, int userId) {
	...
	public CustomResponse synchPharmacyCodeMaintenanceDataWithExistingData(int fileId, int userId) {
		...
		if (codeMaintenanceFile != null) {
			...
			System.out.println(inProcessFilePath);
			try {
				log.info("==== Pharmacy Added Records Synch Started ");
				...
				codeMaintenanceLoggerService.saveCodeMaintenanceLog(fileId,
						"Pharmacies Added Records Synching Completed",
						"Pharmacies Added Records Synching Completed Successfully", userId);
				log.info("===== Pharmacies Added Records Synch Completed ");

				log.info("===== Pharmacy Updated Records Synch Started ");
				...
				codeMaintenanceLoggerService.saveCodeMaintenanceLog(fileId,
						"Pharmacies Updated Records Synching Completed",
						"Pharmacies Updated Records Synching Completed Successfully", userId);
				log.info("===== Pharmacies Updated Records Synch Completed ");

				log.info("===== Pharmacies Delete Records Synch Started ");
				...
				codeMaintenanceLoggerService.saveCodeMaintenanceLog(fileId,
						"Pharmacies Deleted Records Synching Completed",
						"Pharmacies Deleted Records Synching Completed Successfully", userId);
				log.info("===== Pharmacies deleted Records Synch Completed ");
				codeMaintenanceLoggerService.saveCodeMaintenanceLog(fileId, "Pharmacy Data Synching Completed",
						"Pharmacy Data Synching Completed Successfully", userId);
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
...
//    
			utilsService.truncateTable("pharmacy_standard_versions");
			codeMaintenanceFileService.updateCodeMaintenanceFileStatusById(fileId, "Synched", "synching completed",
					userId);
	...
	public Page<PharmacyDataVerificationModel> searchPharmacyFromVerification(Integer file_id, String ncpdp_id,
			String name, String address, String zip, String status, int pageSize, int pageNo) {
		Pageable paging = PageRequest.of(pageNo, pageSize);

		List<PharmacyDataVerificationModel> pharmacyVerificationList = pharmacyDataVerificationRepository
				.getPharmacyVerificationDetails(file_id, ncpdp_id, name, address, zip, status);

//	Page<PharmacyDataVerificationModel> pagedResult = new 
//			PageImpl<PharmacyDataVerificationModel> (pharmacyVerificationList,paging, 
//					pharmacyVerificationList.size());
	...
	protected CustomResponse saveCodeVerificationLogDetails(CodeVerfication codes, String codeset, int fileId,int userId,String notes) {
		CodeMaintenanceFile zipFile = codeMaintenanceFileService.getCodeMaintenanceFileById(fileId);
		int id = 0;
	...
	public List<PharmacyDataVerificationModel> acceptedVerification(List<String> accepted,
		...
		for (String ncpdpCode : accepted) {
			...
			if (model != null) {
				...
				model.setVerificationStatus("Accepted");
	...
	public List<PharmacyDataVerificationModel> rejectedVerification(List<String> rejected,
		...
		for (String ncpdpCode : rejected) {
			...
			if (model != null) {
				...
				model.setVerificationStatus("Rejected");
